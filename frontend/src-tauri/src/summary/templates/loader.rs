...
/// - macOS: ~/Library/Application Support/JameelNote/templates/
/// - Windows: %APPDATA%\JameelNote\templates\
/// - Linux: ~/.config/JameelNote/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push(crate::branding::APP_DIR_NAME);
    path.push("templates");
    Some(path)
}
